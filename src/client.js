...
const getAllFromPackage = async (pkgData, axios, {verbose}) => {
  ...
  const asProjects = packages.map(p => meta.packageRefs[p]);
  const unknown = asProjects.filter(r => !r).length;
  const filtered = asProjects.filter(r => !!r);
  const uniqueRefs = Array.from(new Set(filtered));
  const queued = uniqueRefs.filter(r => !meta.sourceCredRefs[r]).length;
  const usableRefs = uniqueRefs.filter(r => !!meta.sourceCredRefs[r]);

  // Fetches all available scores relevant for this package.json.
  const scoreMap = new Map(await Promise.all(
  ...
  return {scoreMap, unknown, queued};
};
